...
app = FastAPI(title="Marks Extraction API - Hugging Face", description="OCR-based marks extraction service")
...
async def startup_event():
    ...
    print("=" * 60)
    print("Starting Marks Extraction API on Hugging Face Spaces")
    print("=" * 60)
    ...
    try:
        ...
        print("=" * 60)
        print(f"✗ CRITICAL ERROR: Failed to load ML models: {str(e)}")
        print("=" * 60)
        import traceback
        traceback.print_exc()
...
async def root():
    return {
        "message": "Marks Extraction API is running on Hugging Face Spaces",
        "version": "1.0.0",
        "endpoints": {
            "health": "/health",
            "extract_marks": "/api/extract-marks"
        }
    }
...
async def health_check():
    ...
    models_loaded = (layout_model is not None and
                    cell_detection_model is not None and 
                    text_rec_model is not None)
